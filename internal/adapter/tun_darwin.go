...
// TUNAdapter represents a TUN network adapter for macOS
...
// Note: macOS implementation is currently a stub
func NewTUNAdapter(opts *Options) (AdapterInterface, error) {
	return &TUNAdapter{
		name:      opts.Name,
		address:   opts.Address,
		state:     StateError,
		lastError: fmt.Errorf("TUN adapter not implemented for macOS"),
	}, nil
}
