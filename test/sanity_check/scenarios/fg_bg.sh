...
# fg_bg.sh
# Tests SSSonector with server in foreground and client in background mode
set -euo pipefail
...
main() {
    ...
    TEST_DIR="${RESULTS_DIR}/fg_bg_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "${TEST_DIR}"
    ...
    if [[ ${failed} -eq 0 ]]; then
        log_info "Foreground-Background test completed successfully"
    else
        log_error "Foreground-Background test failed"
    fi
