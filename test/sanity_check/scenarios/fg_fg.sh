...
# fg_fg.sh
# Tests SSSonector with both server and client in foreground mode
set -euo pipefail
...
start_server() {
    ...
    ssh "${SSSONECTOR_SERVER_USER}@${SSSONECTOR_SERVER_HOST}" \
        "cd ${SSSONECTOR_DEPLOY_ROOT} && \
         ...
         -config ${SSSONECTOR_DEPLOY_ROOT}/etc/sssonector/server.yaml" \
...
start_client() {
    ...
    ssh "${SSSONECTOR_CLIENT_USER}@${SSSONECTOR_CLIENT_HOST}" \
        "cd ${SSSONECTOR_DEPLOY_ROOT} && \
         sudo ${SSSONECTOR_DEPLOY_ROOT}/bin/sssonector \
         -config ${SSSONECTOR_DEPLOY_ROOT}/etc/sssonector/client.yaml" \
        > "${TEST_DIR}/client.log" 2>&1 &
    
    # Store PID
    echo $! > "${TEST_DIR}/client.pid"
...
cleanup() {
    ...
    if [[ -f "${TEST_DIR}/client.pid" ]]; then
        ...
        ssh "${SSSONECTOR_CLIENT_USER}@${SSSONECTOR_CLIENT_HOST}" "sudo pkill -f sssonector" || failed=1
    ...
    if [[ -f "${TEST_DIR}/server.pid" ]]; then
        ...
        ssh "${SSSONECTOR_SERVER_USER}@${SSSONECTOR_SERVER_HOST}" "sudo pkill -f sssonector" || failed=1
...
main() {
    ...
    TEST_DIR="${RESULTS_DIR}/fg_fg_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "${TEST_DIR}"
    ...
    if [[ ${failed} -eq 0 ]]; then
        log_info "Foreground-Foreground test completed successfully"
    else
        log_error "Foreground-Foreground test failed"
    fi
